...
export function CloudProjectsPanel({ apiFactory, editor, onClose, onSignIn }: {
  ...
  return <div className="modal-backdrop cloud-project-backdrop">
    <section ref={dialog} tabIndex={-1} className="cloud-project-panel" role="dialog" aria-modal="true" aria-label="Projets cloud" onKeyDown={(e) => {
      ...
      <header className="cloud-project-header"><div><p className="cloud-eyebrow">VOTRE ESPACE DE TRAVAIL</p><h2>Projets cloud</h2><p>Vos scénarios, privés ou partagés avec les personnes de votre choix.</p></div><button type="button" aria-label="Fermer les projets cloud" onClick={onClose}><UiIcon name="x"/></button></header>
      {!accountId ? <div className="cloud-empty"><h3>{busy ? 'Connexion à votre espace…' : 'Connectez-vous pour retrouver vos projets'}</h3><p>Les scénarios locaux restent disponibles dans l’éditeur.</p><button disabled={busy} onClick={onSignIn}>Ouvrir mon compte</button></div> : <>
        ...
        <div className="cloud-project-layout"><div className="cloud-project-list">
          ...
          {visible.map((p) => <button key={p.id} className={`cloud-project-card ${selected?.id === p.id ? 'is-selected' : ''}`} disabled={busy} onClick={() => void run(() => select(p))}><UiIcon name="file"/><span><strong>{p.title}</strong><small>{p.sharing === 'private' ? 'Privé · vous uniquement' : `Partagé · ${p.memberCount} membres`} · {roleLabel[p.role]}</small><small>Modifié le {new Date(p.updatedAt).toLocaleDateString('fr-FR')}</small></span><UiIcon name="chevron"/></button>)}
          ...
          {!selected ? <div className="cloud-empty"><h3>Sélectionnez un projet</h3><p>Ouvrez-le, consultez ses versions ou choisissez avec qui le partager.</p></div> : <>
            ...
            {selected.realtimeStudioId && <p className="cloud-help">Le texte, les commentaires et la page de garde sont partagés. Les modifications apparaissent chez les autres membres après quelques secondes. En cas de conflit, votre copie locale est conservée.</p>}
            ...
            {detail && <section><h4>Accès à ce projet</h4>{detail.members.map((m) => <div className="cloud-member" key={m.profileId}><span><strong>{m.displayName}{m.profileId === accountId ? ' (vous)' : ''}</strong><small>{roleLabel[m.role]}</small></span>{detail.studio.role === 'owner' && m.role !== 'owner' && <div><UiSelect aria-label={`Accès de ${m.displayName}`} value={m.role} disabled={busy} onChange={(e) => void run(() => memberAction(m.profileId, e.target.value as 'editor' | 'viewer'))}><option value="viewer">Lecteur</option><option value="editor">Éditeur</option></UiSelect><button disabled={busy} onClick={() => void run(() => memberAction(m.profileId, null))}>Retirer</button></div>}</div>)}
              {detail.studio.role === 'owner' && <form className="cloud-invite-form" onSubmit={(e) => void invite(e)}><label>Inviter par adresse e-mail<input name="email" type="email" required placeholder="nom@exemple.fr" /></label><label>Autorisation<UiSelect aria-label="Autorisation" name="role" defaultValue="viewer"><option value="viewer">Lecteur · consulter</option><option value="editor">Éditeur · modifier</option></UiSelect></label><button disabled={busy}>Créer l’invitation</button></form>}
