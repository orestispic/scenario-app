...
export type AiRequestStatus = "reserved" | "succeeded" | "released" | "uncertain";
...
export type AiActionResult =
  ...
  | { kind: "translations"; translations: Array<{ index: number; text: string }> };
...
function readQuota(value: unknown): AiQuotaView {
  if (
    ...
    throw new Error("Réponse de quota IA invalide.");
  }
...
export function parseAiExecutionResponse<T>(
  ...
  if (
    ...
    !["reserved", "succeeded", "released", "uncertain"].includes(String(value.status)) ||
    typeof value.replayed !== "boolean" ||
    typeof value.request_id !== "string" ||
    !(value.result === null || isRecord(value.result))
  ) {
    throw new Error("Réponse IA commerciale invalide.");
  }
  return { ...value, quota: readQuota(value.quota) } as unknown as AiExecutionResponse<T>;
}
...
export function parseAiReconcileResponse(value: unknown): AiReconcileResponse {
  if (
    ...
    throw new Error("Réconciliation IA invalide.");
  }
  return { ...value, quota: readQuota(value.quota) } as unknown as AiReconcileResponse;
}
